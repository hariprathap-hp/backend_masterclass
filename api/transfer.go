...
func (server *Server) createTransfer(c *gin.Context) {
	...
	fromAccount, valid := server.validateAccountID(c, req.FromAccountID, req.Currency)
	if !valid {
		return
	}
	...
	_, valid = server.validateAccountID(c, req.ToAccountID, req.Currency)
	if !valid {
		return
	}
...
func (server *Server) validateAccountID(c *gin.Context, accountID int64, currency string) (db.Accounts, bool) {
